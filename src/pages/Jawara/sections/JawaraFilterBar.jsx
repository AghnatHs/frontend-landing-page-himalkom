    ...
    <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg border border-primary/40 bg-white shadow-sm text-base font-semibold hover:bg-primary-light focus:outline-none focus:ring-2 focus:ring-primary-dark min-w-[180px]"
        >
        ...
        {dropdownOpen && (
          <div className="absolute z-20 mt-2 w-64 max-h-72 overflow-y-auto bg-white border border-primary/30 rounded-xl shadow-lg p-3 flex flex-col gap-2 animate-fadeIn">
            {communities.map((kom) => (
              <label key={kom.slug} className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all hover:bg-primary-light ${selectedCommunities.includes(kom.slug) ? 'ring-2 ring-primary-dark bg-primary-light/60' : ''}`}>
                <input
                  type="checkbox"
                ...
                <span className="font-semibold text-primary-darker text-sm">{kom.name}</span>
              </label>
            ))}
          </div>
        )}
